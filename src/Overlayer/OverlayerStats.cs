...
namespace BypassedResourcePack
{
    ...
    //  - Judgement counts come straight from the game's mistakesManager.GetHits (it already
    //    resets per attempt), matching Overlayer's official OTE/OVE/... and Miss/Overload tags.
    ...
    internal static class OverlayerStats
    {
        ...
        // =====================================================================
        // Judgement counts — tallied from the scrMarginTracker.AddHit hook, reset each run.
        // (This game build's scrMistakesManager has no GetHits, so we count ourselves.)
        // =====================================================================
        ...
        {
            int i = (int)m;
            return (i >= 0 && i < counts.Length) ? counts[i] : 0;
        }
        ...
        internal static void OnRunStart()
        {
            ...
            Combo = 0;
            IsStarted = true;
            // Force Tabub to re-init next tick. Restarting while still alive never toggles
            // IsStarted, so Tabub's own reset (on IsStarted false->true) wouldn't fire.
            tabStarted = false;
