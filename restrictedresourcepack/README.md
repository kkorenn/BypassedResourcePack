...
- `Deafen at (%)` — default `5`; once an eligible live run reaches this progress, the mod sends `SET_VOICE_SETTINGS { deaf: true }`; when stopping/unloading or leaving the run, it sends `deaf: false`.
- `Only when starting from 0%` — matches KorenResourcePack's default behavior and prevents mid-map practice/checkpoint starts from triggering deafen.

Enable the feature, enter your Discord app **Client ID**, then press **Open Discord** or **Copy URL** in the settings panel. The redirect URI is fixed:
